...
int main()
{
  ...
  REP(i, n) {
    if (bio[p[i]]++) continue;
    if (a[p[i]+1]) {
      if (2*p[i] >= MAXM || a[2*p[i]] != a[p[i]+1])
        V[a[p[i]+1] - p[i]].push_back(P(i, ozn[a[p[i]+1]]));
    }

    for (int j=2*p[i]; j<MAXM && a[j]; j+=p[i])
      if (j + p[i] >= MAXM || a[j+p[i]] != a[j])
        V[a[j]-j].push_back(P(i, ozn[a[j]]));
  }
