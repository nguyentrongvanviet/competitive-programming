...
void doc()
{
	...
	if(n%2==0)
	{
		for(int x= n/2 , y =n/2+1 ; x>=1&&y<=n;x--,y++)
		{
			if(!(pos[x]<=pos[n/2]&&pos[y]>=pos[n/2+1]))return void(cout<<x<<el);
		}
		for(int x = n/2 , y=n/2+1;x>=1&&y<=n;x--,y++)
		{
	...
	{
		for(int x= n/2,y=n/2+2;x>=1&&y<=n;x--,y++)
		{
			if(!(pos[x]<=pos[n/2]&&pos[y]>=pos[n/2+2]))
			{
				return void(cout<<x<<el);
			}
		}
	}
	cout<<0<<el;
