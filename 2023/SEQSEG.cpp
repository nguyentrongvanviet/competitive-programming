...
#define    FORDb(i,a,b)  for(int i=(int)(a);i>=(int)(b);i--)
...
namespace sub2
{
	...
	void solve(int k, int l, int r, int oL , int oR)
	{
		...
		FOR(i,oL,min(mid,oR))
		{
			if(maxi(f[mid][k],f[i-1][k-1]+cost(i-1,mid)))pos=i;
		}
		solve(k,l,mid-1,oL,pos) ; 
		solve(k,mid+1,r,pos,oR) ; 
	}
	// vị trí j là vị trí cập nhật cho f[j][k-1] -> f[i][k]  thì những thằng f[i+1][k] sẽ có vị trí tối ưu là từ j đổ lên 
