	...
	void xuly()
	{
		while(q--)
		{
			...
			if(TYPE=='S')
			{
				...
				FOR(i,l,r)g[i]=G,c[i]=C ; 
			}
			else
			{
				int l ,r , k; cin>>l>>r>>k ; 
				FOR(i,0,k-1)f[l-1][i] = 0 ;
				f[l-1][0] =1 ; 
				int res= 1 ; 
				FOR(i,l,r)
				{
					res=1LL*res*(g[i]+c[i])%sm;
					f[i][0] = 1LL*f[i-1][0]*c[i]%sm;
					FOR(j,1,k-1)
					{
						f[i][j] = (1LL*f[i-1][j]*c[i]%sm+1LL*f[i-1][j-1]*g[i]%sm)%sm;
					}
				} 
				FOR(i,0,k-1)
				{
...
{	
    ...
    {
    	if(L.num_way==0)return R ; 
    	if(R.num_way==0)return L ;
        Node ans; 
