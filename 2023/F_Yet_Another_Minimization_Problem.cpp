...
namespace sub1
{
	...
	void xuly()
	{
		...
		FOR(j,2,k)
		{
			FOR(i,j,n)
			{
				FOR(ii,1,i)sl[a[ii]] = 0 ; 
				ll tmp = 0 ; 
				FORD(ii,i,1)
				{
					tmp+=sl[a[ii]];
					sl[a[ii]]++;
					mini(f[i][j],f[ii-1][j-1]+tmp) ;
				}
			}
		}
		cout<<f[n][k] ;
...
namespace sub2
{
	...
	{	
		while(R<r)cost+=sl[a[++R]]++; 
		while(l<L)cost+=sl[a[--L]]++;
		while(r<R)cost-=--sl[a[R--]];
		while(L<l)cost-=--sl[a[L++]];
	}
	void solve(int l, int r ,int oL , int oR ,int k)
	{
		...
		{ 
			update(i,mid) ; 
			if(f[i-1][k-1]+cost<=f[mid][k])
			{
				f[mid][k] = f[i-1][k-1]+cost ; 
				pos = i; 
			}
		}
