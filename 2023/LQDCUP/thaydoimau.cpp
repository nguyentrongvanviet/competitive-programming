...
const int N = 1e5+5 , oo = 1e9+5 , LO = 17 , CH = 26 ; 
...
namespace sub2
{
	...
	struct DL
	{
		int L , R; 
		int ans ;
		DL(int _L=2*n,int _R=-2*n,int _ans=4*n)
		{
			L=_L ;
			R=_R;
			ans=_ans;
		}
		bool operator !=(const DL&a)
		{
			return L!=a.L||R!=a.R||ans!=a.ans;
		}	
	} ;
	DL merge(DL L , DL R)
	{
