...
namespace sub1
{
	struct DG
	{
		...
		{
			return val>a.val||(val==a.val&&cl<a.cl) ; 
		}
	} ; 
	struct BG
	{
	...
	void Q()
	{
		...
		while(!q.empty())
		{
			...
			for(auto x:g[u])
			{
				int v=x.v ; 
				int w =x.w ; 
				if(cl>=w&&mini(f[v],{val,cl-w}))q.push({v,f[v]}) ; 
				else if(X>=w&&mini(f[v],{val+1,X-w}))q.push({v,f[v]}) ; 
			}
		}
	}
	struct DL
    ...
    {		
    	...
    	FOR(i,1,n)
    	{
    		int x = max(0ll,(d[i]-f[i].cl)/X+((d[i]-f[i].cl)%X!=0)-a[i]);
    		mini(res,f[i].val+x) ;  ;
    	}
    	cout<<res; 
    }
