...
namespace sub1
{
    ...
    {   
    	...
    	FOR(i,1,n)
    	{
    		int x = UB(maL+1,maL+i+1,maR[i+1])-maL-1;
    		int y = UB(miL+1,miL+i+1,miR[i+1],[&](int val ,int j){return j<val;})-miL;
    		int u = min(x,y) ; 
    		if(u&&!(i==n&&u==1))
    		{
    			mini(res,a[i].c-a[u].c+maR[i+1]-miR[i+1]);
    		}
    		u = max(x+1,y+1) ; 
    		mini(res,a[i].c+get(1,u,i)) ;

    		mini(res,a[i].c+maR[i+1]+get(2,y+1,x)) ; 
    		mini(res,a[i].c+miR[i+1]+get(3,x+1,y)) ; 
    	}
    	cout<<res<<el;
    }	
}
...
{   
    ...
    void xuly()
    {
    	...
    	{
    		if(i==1&&j==n)continue ; 
    		mini(res,a[j].c-a[i].c+max(maL[j+1],maR[i-1])-min(miL[j+1],miR[i-1]) ) ; 
    	}
    	cout<<res ;
    }
